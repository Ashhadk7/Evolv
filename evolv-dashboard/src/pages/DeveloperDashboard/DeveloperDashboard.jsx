...
import styles2 from '../../components/FeaturedMatch/FeaturedMatch.module.css';
import styles3 from '../../components/MatchCard/MatchCard.module.css';
import styles4 from '../../components/ProfileCard/ProfileCard.module.css';
...
const MODALS = {
    viewAllMatches: {
        ...
        badge: 'Discover Page',
        description: 'Browse your full list of AI-curated startup matches ranked by compatibility. Filter by industry, stage, budget, and tech stack.',
        details: [
    ...
    viewAllApplications: {
        ...
        description: 'Track the full pipeline of your startup applications — from submitted to interview to accepted. See status updates in real-time.',
        details: [
    ...
    viewAllProjects: {
        ...
        badge: 'Projects Page',
        description: 'Monitor progress, deadlines, team collaboration, and hours logged across all your active startup engagements.',
        details: [
    ...
    shareMatch: {
        ...
        title: 'Share Match',
        badge: 'Share Feature',
        description: 'Share this startup match with a colleague or mentor via a shareable link, email, or export a summary PDF.',
        details: [
    ...
    editProfile: {
        ...
        badge: 'Settings Page',
        description: 'Update your skills, portfolio links, availability status, and bio. A complete profile increases your AI match score significantly.',
        details: [
    ...
    viewPortfolio: {
        ...
        badge: 'Portfolio',
        description: "Sarah's public portfolio showcases 12 projects spanning AI/ML systems, full-stack applications, and open-source contributions.",
        details: [
    ...
    linkedin: {
        ...
        description: 'Connect with Sarah on LinkedIn to see her full work history, recommendations, and professional endorsements.',
        details: [
    ...
    twitter: {
        ...
        title: 'Twitter / X',
        badge: 'Social',
        description: 'Follow Sarah for AI/ML thoughts, startup insights, and tech threads. 2.1K followers and counting.',
        details: [
    ...
    notification: {
        ...
        title: 'Notifications',
        badge: '3 Unread',
        description: 'You have 3 unread notifications — an interview invite from Nexus Health, a match score update, and a project deadline reminder.',
        details: [
    ...
    applicationItem: (app) => ({
        icon: app.icon,
        accent: app.status === 'Accepted' ? '#5BC8A0'
              : app.status === 'Interview' ? '#C4973A'
              : app.status === 'Declined' ? '#FF6B6B'
              : '#888',
        title: app.name,
        badge: app.status,
        ...
        steps:
            app.status === 'Interview'
                ? ['Prepare for your interview', "Review the startup's tech stack", 'Confirm the meeting time']
                : app.status === 'Accepted'
                ? ['Congratulations! Review the offer', 'Discuss terms with the founder', 'Accept or negotiate']
                : app.status === 'Declined'
        ...
        cta: app.status === 'Interview' ? 'Prepare Interview'
           : app.status === 'Accepted'  ? 'View Offer'
           : 'View Application',
    }),
};
        ...
        <div className={styles.dashboardContainer}>
            ...
            <main className={styles.mainWrapper}>
                ...
                <div className={styles.row2}>
                    <div className={styles.latestMatchWrapper}>
                ...
                <div className={styles.row3}>
                    <div className={styles.sectionHeader}>
                        ...
                        <a href="#" onClick={(e) => { e.preventDefault(); openModal(MODALS.viewAllMatches); }}>View all matches →</a>
                    </div>
                ...
                <div className={styles.row4}>
                    <div className={styles.panel}>
                        <div className={styles.panelHeader}>
                            ...
                            <a href="#" className={styles.panelLink} onClick={(e) => { e.preventDefault(); openModal(MODALS.viewAllApplications); }}>View all →</a>
                        </div>
                        {applications.map((app) => (
                            <div key={app.id} style={{ cursor: 'pointer' }} onClick={() => openModal(MODALS.applicationItem(app))}>
                    ...
                    <div className={styles.panel}>
                        <div className={styles.panelHeader}>
                            ...
                            <a href="#" className={styles.panelLink} onClick={(e) => { e.preventDefault(); openModal(MODALS.viewAllProjects); }}>View all →</a>
                        </div>
...
const FeaturedMatchWithModal = ({ data, openModal }) => {
    ...
    return (
        <div className={styles2.featuredMatch}>
            <div className={styles2.sectionTop}>
                <i className="fas fa-fire" style={{ color: '#FF6B6B' }}></i>
                <span className={styles2.secLabel}>Latest AI Match</span>
            </div>
            <div className={styles2.card}>
                ...
                <div className={styles2.body}>
                    <div className={styles2.topRow}>
                        <span className={styles2.name}>{name}</span>
                        <span className={styles2.score}>{matchScore}% Match</span>
                    </div>
                    <div className={styles2.desc}>{description}</div>
                    <div className={styles2.techTags}>
                        {techStack.map((tech, idx) => (
                            <span key={idx} className={styles2.techTag}>{tech}</span>
                        ))}
                    </div>
                    <div className={styles2.whyBox}>
                        <div className={styles2.whyLabel}>Why matched?</div>
                        <ul>
...
const MatchCardWithModal = ({ data, openModal }) => {
    ...
    return (
        <div className={styles3.matchCard}>
            <div className={styles3.top}>
                <div className={styles3.left}>
                    <div className={`${styles3.icon} ${iconClass ? styles3[iconClass] : ''}`}>
                        <i className={`fas fa-${icon}`}></i>
                    </div>
                    <span className={styles3.name}>{name}</span>
                </div>
                <span className={styles3.score}>{matchScore}%</span>
            </div>
            <div className={styles3.desc}>{description}</div>
            <div className={styles3.tags}>
                {techStack.map((tech, idx) => (
                    <span key={idx} className={styles3.techTag}>{tech}</span>
                ))}
            </div>
            <div className={styles3.meta}>
            ...
            <div className={styles3.actions}>
                <button
                    className={`${styles3.btn} ${styles3.primary}`}
                    onClick={() => openModal(MODALS.reviewMatch(data))}
                >
...
const ProfileCardWithModal = ({ data, openModal }) => {
    ...
    return (
        <div className={styles4.profileCard}>
            <div className={styles4.header}>
                <span className={styles4.title}><i className="fas fa-user-circle"></i> Developer Profile</span>
                <a href="#" onClick={(e) => { e.preventDefault(); openModal(MODALS.editProfile); }}>Edit Profile</a>
            </div>
            <div className={styles4.content}>
                <div className={styles4.avatarWrap}>
                    <img src={image} alt={name} className={styles4.avatar} />
                    <div className={styles4.statusDot}></div>
                </div>
                <div className={styles4.right}>
                    <div className={styles4.name}>{name}</div>
                    <div className={styles4.role}>{role}</div>
                    <div className={styles4.meta}>
                        <div className={styles4.metaItem}><i className="fas fa-map-marker-alt"></i> {location}</div>
                        <div className={styles4.metaItem}><i className="fas fa-briefcase"></i> {experience}</div>
                        <div className={`${styles4.metaItem} ${styles4.avail}`}><i className="fas fa-circle"></i> Availability: {availability}</div>
                    </div>
                </div>
            </div>
            <div className={styles4.stats}>
                <div className={styles4.stat}>
                    <span className={styles4.statLabel}>Match Rate</span>
                    <div className={styles4.statNum}>{matchRate}</div>
                </div>
                <div className={styles4.stat}>
                    <span className={styles4.statLabel}>Profile Strength</span>
                    <div className={styles4.statNum}>{profileStrength}</div>
                </div>
                <div className={styles4.stat}>
                    <span className={styles4.statLabel}>Founder Rating</span>
                    <div className={styles4.statNum}><span className={styles4.star}>★</span> {founderRating}</div>
                </div>
            </div>
            <div className={styles4.bottomRow}>
                <button className={styles4.portfolioBtn} onClick={() => openModal(MODALS.viewPortfolio)}>
                    <i className="fas fa-briefcase"></i> View Portfolio
                </button>
                <div className={styles4.socials}>
