                ...
                <div className={styles.mainGrid}>
                        ...
                        {pendingRequests.length > 0 && (
                            <div className={styles.requestsBox}>
                                <div className={styles.boxTitle}>
                                    ...
                                    Connection Requests
                                    <span className={styles.requestCount}>{pendingRequests.length}</span>
                                </div>
                                <div className={styles.requestList}>
                                    {pendingRequests.map((req) => (
                                        <div key={req.id} className={styles.requestCard}>
                                            <div className={styles.reqAvatar} style={{ background: req.avatarColor }}>{req.avatar}</div>
                                            <div className={styles.reqInfo}>
                                                <div className={styles.reqName}>{req.name}</div>
                                                <div className={styles.reqRole}>{req.role} · {req.company}</div>
                                                <div className={styles.reqMutual}><i className="fas fa-users"></i> {req.mutual} mutual connections</div>
                                            </div>
                                            <div className={styles.reqActions}>
                                                <button className={styles.acceptBtn} onClick={() => handleAcceptRequest(req.id)}>Accept</button>
                                                <button className={styles.ignoreBtn} onClick={() => handleAcceptRequest(req.id)}>Ignore</button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ...
                            {filteredConnections.map((conn) => (
                                <div key={conn.id} className={styles.connectionCard}>
                                    <div className={styles.ccTop}>
                                        <div className={styles.ccAvatar} style={{ background: conn.avatarColor }}>{conn.avatar}</div>
                                        <div className={styles.ccInfo}>
                                            <div className={styles.ccName}>{conn.name}</div>
                                            <div className={styles.ccRole}>{conn.role}</div>
                                            <div className={styles.ccCompany}><i className="fas fa-building"></i> {conn.company}</div>
                                        </div>
                                        <span className={`${styles.typeTag} ${conn.type === 'Founder' ? styles.founderTag : styles.devTag}`}>
                                            {conn.type}
                                        </span>
                                    </div>
                                    ...
                                    <div className={styles.ccActions}>
                                        <button
                                            className={connected[conn.id] ? styles.connectedBtn : styles.connectBtn}
                    ...
                    <div className={styles.rightCol}>
                        ...
                        <div className={styles.sideCard}>
                            <div className={styles.sideCardTitle}><i className="fas fa-bolt"></i> Activity Feed</div>
                            <div className={styles.feedList}>
                                {activityFeed.map((item) => (
                                    <div key={item.id} className={styles.feedItem}>
                                        <div className={styles.feedAvatar} style={{ background: item.color }}>{item.avatar}</div>
                        ...
                        {/* Recommended */}
                        <div className={styles.sideCard}>
                            <div className={styles.sideCardTitle}><i className="fas fa-star"></i> Recommended for You</div>
                            <div className={styles.recommendedList}>
                                {connections.filter((c) => !connected[c.id]).slice(0, 3).map((c) => (
                                    <div key={c.id} className={styles.recommendedCard}>
                                        <div className={styles.recAvatar} style={{ background: c.avatarColor }}>{c.avatar}</div>
                                        <div className={styles.recInfo}>
                                            <div className={styles.recName}>{c.name}</div>
                                            <div className={styles.recRole}>{c.role}</div>
                                            <div className={styles.recMutual}>{c.mutual} mutual connections</div>
                                        </div>
                                        <button className={styles.connectBtnSm} onClick={() => handleConnect(c.id)}>
                                            {connected[c.id] ? '✓' : '+'}
                                        </button>
                                    </div>
                                ))}
                        ...
                        {/* Network Stats */}
                        <div className={styles.sideCard}>
                            <div className={styles.sideCardTitle}><i className="fas fa-chart-pie"></i> Network Breakdown</div>
                            <div className={styles.breakdownList}>
                                <div className={styles.breakdownItem}>
                                    <div className={styles.breakdownLabel}><span className={styles.bdDot} style={{ background: '#5BC8A0' }}></span> Developers</div>
                                    <div className={styles.breakdownBar}><div className={styles.breakdownFill} style={{ width: '66%', background: '#5BC8A0' }}></div></div>
                                    <div className={styles.breakdownVal}>94</div>
                                </div>
                                <div className={styles.breakdownItem}>
                                    <div className={styles.breakdownLabel}><span className={styles.bdDot} style={{ background: '#C4973A' }}></span> Founders</div>
                                    <div className={styles.breakdownBar}><div className={styles.breakdownFill} style={{ width: '27%', background: '#C4973A' }}></div></div>
                                    <div className={styles.breakdownVal}>38</div>
                                </div>
                                <div className={styles.breakdownItem}>
                                    <div className={styles.breakdownLabel}><span className={styles.bdDot} style={{ background: '#7C5CBF' }}></span> Investors</div>
                                    <div className={styles.breakdownBar}><div className={styles.breakdownFill} style={{ width: '7%', background: '#7C5CBF' }}></div></div>
                                    <div className={styles.breakdownVal}>10</div>
                                </div>
                            </div>
                        </div>
                    </div>
