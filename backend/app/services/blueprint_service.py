...
def submit_pending_version(
    db: Session, blueprint_id: UUID, current_user: User, payload: BlueprintVersionCreate
) -> BlueprintVersion:
    blueprint = get_blueprint(db, blueprint_id, current_user, require_ownership=True)
...
def list_versions(db: Session, blueprint_id: UUID, current_user: User) -> list[BlueprintVersion]:
    blueprint = get_blueprint(db, blueprint_id, current_user, require_ownership=True)
    return sorted(blueprint.versions, key=lambda version: version.state.value)


def get_latest_version(db: Session, blueprint_id: UUID, current_user: User) -> BlueprintVersion:
...
def promote_pending_version(
    db: Session, blueprint_id: UUID, current_user: User
) -> BlueprintVersion:
    blueprint = get_blueprint(db, blueprint_id, current_user, require_ownership=True)
