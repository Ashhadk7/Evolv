...
def generate_chat(system: str, messages: list[dict]) -> str:
    ...
    api_key = settings.GROQ_API_KEY.get_secret_value().strip()
    if not api_key:
        raise AgentClientError("GROQ_API_KEY is required for chat completions.")
        ...
        for attempt in range(5):
            try:
                ...
                if attempt < 4:
                    ...
                    continue
    raise AgentClientError(f"Groq returned an invalid chat response: {last_error}") from last_error
