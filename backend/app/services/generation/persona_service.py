...
async def generate_persona_for_blueprint(
    db: Session,
    blueprint_id: UUID,
    current_user: User,
) -> tuple[BlueprintVersion, PersonaOutput]:
    ...
    try:
        persona = await run_persona(source_version.idea_desc, source_version.industry)
    except ValueError as exc:
        raise BlueprintAgentInputError(str(exc)) from exc
    except AgentClientError as exc:
        raise BlueprintGenerationError(
            "Persona agent could not generate output. Check GROQ_API_KEY and model availability."
        ) from exc
    ...
    try:
        ...
        target_version.content_json = merge_persona_content(
        ...
        db.rollback()
        raise BlueprintPersistenceError("Persona output could not be saved.") from exc

    return target_version, persona


def merge_persona_content(
    ...
    agents["persona"] = persona.model_dump(by_alias=True)
