...
Then return outOfScope items that should wait until after MVP validation, the dataEntities and their key fields, the nonFunctional requirements this product must meet, and the build phases whose weeks fit the build timeline stated above and whose `features` list says which features ship in each phase.
