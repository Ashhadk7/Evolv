...
def create_founder_profile(
    payload: FounderProfileCreate, db: DbSession, current_user: CurrentUser
) -> FounderProfileResponse:
    return founder_profile_service.create_profile(db, payload, current_user)


@router.get("", response_model=FounderProfileResponse)
def get_my_founder_profile(db: DbSession, current_user: CurrentUser) -> FounderProfileResponse:
...
def update_founder_profile(
    payload: FounderProfileUpdate, db: DbSession, current_user: CurrentUser
) -> FounderProfileResponse:
    return founder_profile_service.update_profile(db, payload, current_user)


@router.delete("", status_code=status.HTTP_204_NO_CONTENT)
def delete_founder_profile(db: DbSession, current_user: CurrentUser) -> None:
