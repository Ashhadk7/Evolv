...
import type { Blueprint } from "./WorkspaceTab";
...
/* ═══════════════════════════════════════════════════════ */
export type Persona = { name: string; segment: "Primary user" | "Economic buyer" | "Gatekeeper"; about: string; goals: string; pains: string };
...
export type StackLayerKey = "frontend" | "backend" | "database" | "vectorDb" | "aiProvider" | "hosting";
export type TechStackLayer = { chosen: string; options: string[]; reasoning: string; monthlyCost: string };
export type TechStackModel = Record<StackLayerKey, TechStackLayer>;

export type ArchitectureNode = { id: string; label: string; kind: "frontend" | "backend" | "ai" | "data" | "integration" | "infra" };
...
export type StackCat = { icon: ReactNode; name: string; primary: string; rows: { k: string; v: string }[] };
...
function barrierMultiplier(barriers: string): number {
  ...
  if (b.includes("high") || b.includes("regulatory") || b.includes("hardware") || b.includes("grid")) return 0.76;
...
function derivePersonas(bp: Blueprint): Persona[] {
  ...
  if (ind.includes("health")) return [
    { name: "The Frontline Clinician", segment: "Primary user", about: "Oncologists and radiologists who review scans every day and carry the diagnostic decision.", goals: "Faster, more confident diagnoses with fewer false positives.", pains: "Manual review is slow, and existing AI tools don't explain their reasoning." },
    { name: "The Clinic Operations Lead", segment: "Economic buyer", about: "Runs throughput and budget for the practice, and signs off on any new tooling.", goals: "Higher patient throughput with a clear, provable ROI on new tools.", pains: "Cost is hard to justify and EHR integration stalls every rollout." },
    { name: "The Health-System IT Owner", segment: "Gatekeeper", about: "Owns security, compliance and deployment — and can block adoption outright.", goals: "Secure, compliant, low-maintenance software that passes review.", pains: "Data privacy, audit trails and vendor risk are dealbreakers." },
  ];
  return [
    { name: `The ${bp.industry} Practitioner`, segment: "Primary user", about: `The person who would use ${bp.name} day to day to get the core ${bp.industry.toLowerCase()} job done.`, goals: `Get the core ${bp.industry.toLowerCase()} outcome faster, with far less friction.`, pains: "Today's tools are clunky, generic and slow to deliver real value." },
    { name: "The Team Lead", segment: "Economic buyer", about: "Owns the budget and the decision to adopt on behalf of their team.", goals: "Clear ROI, easy rollout and measurable impact within a quarter.", pains: "Hard to evaluate, and switching costs feel high." },
    { name: "The Ops / IT Owner", segment: "Gatekeeper", about: "Responsible for security, access and keeping existing systems reliable.", goals: "Reliable, secure software that fits the systems they already run.", pains: "Security review, permissions and integration overhead slow everything down." },
  ];
}
...
function deriveMarketAnalysis(bp: Blueprint): MarketAnalysis {
  const cagrNum = parseFloat(bp.market.cagr) || 20;
  const demandLevel: MarketAnalysis["demandLevel"] = bp.market.score >= 75 ? "High" : bp.market.score >= 60 ? "Medium" : "Low";
  ...
  return {
    ...
    demandSignals: [
      ...
      bp.competitors.length ? "Comparable competitors validate buyer budget" : "Clear whitespace, but fewer competitor proof points",
    ],
...
function deriveCompetitors(bp: Blueprint): CompetitorRow[] {
  const rows = bp.competitors.length ? bp.competitors : [{ name: "Established incumbent", type: "Direct" }];
  return rows.map((c) => {
    const direct = c.type === "Direct";
    return {
      name: c.name,
      pricing: direct ? "Enterprise pricing, custom quotes" : "Mid-market, seat-based pricing",
      strengths: direct ? ["Established brand trust", "Deep enterprise integrations"] : ["Broad platform reach", "Lower switching friction for buyers"],
      weaknesses: direct ? ["Slow to adapt, built for large teams", "Expensive for smaller buyers"] : [`Generic — not purpose-built for ${bp.industry}`, "Limited depth on the core workflow"],
      gap: direct ? `Priced out of reach for smaller ${bp.industry} teams` : "Doesn't go deep enough on the core workflow",
    };
  });
}
...
const SIMILAR_STARTUPS_BY_INDUSTRY: Record<string, SimilarStartup[]> = {
  health: [
    { name: "PathAI", oneLiner: "AI-assisted pathology diagnostics for hospital labs.", outcome: "Raised $255M+ across multiple rounds; deployed across major health systems." },
    { name: "Tempus", oneLiner: "Precision-medicine platform combining clinical and molecular data.", outcome: "IPO'd in 2024 at a multi-billion dollar valuation." },
    { name: "Paige", oneLiner: "FDA-cleared AI for cancer diagnosis from pathology slides.", outcome: "First FDA-approved AI product in digital pathology." },
  ],
  saas: [
    { name: "Linear", oneLiner: "Opinionated issue tracking built for fast-moving product teams.", outcome: "Reached a $1.25B valuation with a small, focused team." },
    { name: "Notion", oneLiner: "Flexible workspace for docs, wikis, and lightweight databases.", outcome: "Grew to 30M+ users before raising at a $10B valuation." },
  ],
  clean: [
    { name: "Arcadia", oneLiner: "Software layer connecting households and businesses to clean energy.", outcome: "Raised $377M+ and partners with utilities across the US." },
    { name: "LO3 Energy", oneLiner: "Blockchain-based peer-to-peer energy trading.", outcome: "Piloted transactive-grid projects with utilities in three countries." },
  ],
  ed: [
    { name: "Khanmigo", oneLiner: "AI tutor layered on Khan Academy's curriculum.", outcome: "Rolled out to hundreds of school districts within its first year." },
    { name: "Squirrel AI", oneLiner: "Adaptive learning engine for K-12 math in China.", outcome: "Scaled to 2,000+ learning centers before expanding internationally." },
  ],
};
function deriveSimilarStartups(bp: Blueprint): SimilarStartup[] {
  ...
  return [
    { name: `${bp.industry} category leader`, oneLiner: `An established player proving demand for ${bp.industry.toLowerCase()} tooling exists.`, outcome: "Reached meaningful scale with a comparable product thesis." },
    { name: "Adjacent-market challenger", oneLiner: `A startup that broke into ${bp.industry.toLowerCase()} from a neighbouring category.`, outcome: "Validated that outsiders can win here with the right wedge." },
  ];
}
...
function deriveTechStack(bp: Blueprint): TechStackModel {
  ...
  return {
    frontend: { chosen: frontend, options: STACK_OPTIONS.frontend, reasoning: `${frontend} gives the fastest path to a polished, responsive UI with a large hiring pool.`, monthlyCost: "Included in hosting" },
    backend: { chosen: backend, options: STACK_OPTIONS.backend, reasoning: `${backend} fits the ${bp.industry} domain logic and scales cleanly as usage grows.`, monthlyCost: "Included in hosting" },
    database: { chosen: database, options: STACK_OPTIONS.database, reasoning: `${database} handles relational data with strong consistency guarantees for this domain.`, monthlyCost: "$15–60/mo (managed)" },
    vectorDb: { chosen: vectorDb, options: STACK_OPTIONS.vectorDb, reasoning: `${vectorDb} is the fastest way to ship semantic search and retrieval without managing infra.`, monthlyCost: "$0–70/mo (free tier available)" },
    aiProvider: { chosen: aiProvider, options: STACK_OPTIONS.aiProvider, reasoning: `${aiProvider} gives the best balance of quality, latency, and cost for this use case.`, monthlyCost: "$50–400/mo (usage-based)" },
    hosting: { chosen: hosting, options: STACK_OPTIONS.hosting, reasoning: `${hosting} keeps deploys simple with zero-ops scaling for an early-stage team.`, monthlyCost: bp.cost.hosting },
  };
}
...
const DIAGRAM_NODE_ORDER: { id: string; kind: ArchitectureNode["kind"]; layerKey: StackLayerKey | null; fixedLabel?: string }[] = [
...
export function buildArchitecture(techStack: TechStackModel): { nodes: ArchitectureNode[]; edges: ArchitectureEdge[] } {
  ...
  const nodes: ArchitectureNode[] = DIAGRAM_NODE_ORDER.map((n) => {
    ...
    const prefix = { client: "Client", api: "API", ai: "AI Layer", vector: "Vector Store", db: "Database", hosting: "Hosting" }[n.id] || n.id;
...
function buildPhases(bp: Blueprint): Phase[] {
  const raw: Omit<Phase, "primarySkill" | "weeklyRate" | "cost">[] = [
    { name: "Foundation & UI", layer: bp.techStack.frontend, weeks: 3,
      deliverables: ["Design system & component library", "Responsive app shell", "Auth & onboarding screens"],
      acceptanceCriteria: ["Design system covers every core screen", "Auth flow works end-to-end in staging"],
      skillset: ["Frontend", bp.techStack.frontend.split(",")[0].trim()], status: "In Progress" },
    { name: "Core Backend & API", layer: bp.techStack.backend, weeks: 4,
      deliverables: ["Data model & migrations", "REST API + auth middleware", "Core business logic"],
      acceptanceCriteria: ["API contract documented and typed end-to-end", "Core business logic covered by tests"],
      skillset: ["Backend", bp.techStack.backend.split(",")[0].trim()], status: "Planned" },
    { name: "Intelligence Layer", layer: bp.techStack.ai, weeks: 3,
      deliverables: ["Model integration", "Inference pipeline & caching", "Results & insights API"],
      acceptanceCriteria: ["Inference latency within target on staging data", "Results API returns explainable output"],
      skillset: ["AI/ML", bp.techStack.ai.split(",")[0].trim()], status: "Planned" },
    { name: "Payments & Integrations", layer: "Stripe Connect", weeks: 2,
      deliverables: ["Milestone escrow flow", "Connected accounts & payouts", "Email & notifications"],
      acceptanceCriteria: ["A milestone can be funded, approved, and paid out end-to-end in test mode"],
      skillset: ["Backend", "Stripe Connect"], status: "Planned" },
    { name: "Hardening & Launch", layer: "QA · CI/CD", weeks: 2,
      ...
      skillset: ["DevOps", "QA"], status: "Planned" },
  ];
  return raw.map((p) => {
...
function buildCostModel(bp: Blueprint, phases: Phase[]): CostModel {
  ...
  return {
    devTotal, platformFeePct, platformFee, devTakeHome, infraDuringBuild, contingency, total,
    buildWeeks,
...
function deriveFinancials(bp: Blueprint, buildCost: number): Financials {
  ...
  return {
    ...
    startingUsers,
    monthlyGrowthPct,
    revenueModel: "B2B SaaS subscription with usage-based tiers. Founders fund development directly; developers are paid per approved milestone through Evolv's Stripe Connect escrow.",
    year1,
    eoyMrr: eoy.mrr,
    eoyArr: eoy.mrr * 12,
    breakEvenMonth,
