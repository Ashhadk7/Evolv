...
export function Logo({ dark = false, compact = false }: { dark?: boolean; compact?: boolean }) {
  return (
    <Link href="/" className={`inline-flex w-fit items-center ${compact ? "gap-2" : "gap-2.5"}`}>
      <Image
        ...
        className={`${compact ? "h-9 w-12" : "h-12 w-16"} shrink-0 object-contain`}
      />
      <span
        className={`font-bold tracking-tight ${compact ? "text-[18px]" : "text-[20px]"}`}
        style={{ color: dark ? BRAND_CREAM : BRAND_INK }}
      >
