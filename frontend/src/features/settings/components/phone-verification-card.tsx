...
export function PhoneVerificationCard({
  ...
  useEffect(() => {
    ...
    getPhoneStatus(session.accessToken)
      .then((response) => {
        if (!alive) return;
        const next = toPhoneVerificationStatus(response);
        setStatus(next);
        setPhone(next.phone ?? initialPhone);
        savePhoneStatusToLocalProfile(session.role, next, session.email);
  ...
  const verifyCode = async () => {
    ...
    try {
      ...
      const response = await verifyPhone({ firebase_id_token: firebaseIdToken }, session.accessToken);
      const next = toPhoneVerificationStatus(response);
      setStatus(next);
    ...
    <section className="border-y border-[#edf1ee] py-5">
      <div className="mb-4 flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h5 className="text-[13px] font-extrabold text-[#1a2e26]">
              Phone verification
            </h5>
