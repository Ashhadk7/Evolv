...
type ActionModalProps = {
  modal: ActionModalData | null;
  onClose: () => void;
};

export const ActionModal = ({ modal, onClose }: ActionModalProps) => {
  useEffect(() => {
    if (!modal) return;
  ...
  return (
    <div className={styles.overlay} onClick={onClose}>
      <div
        ...
        style={{ "--accent": modal.accent || "#5BC8A0" } as CSSProperties}
      >
        ...
        <div className={styles.footer}>
          <button className={`${devPrimaryBtn.button} ${styles.primaryBtn}`} onClick={onClose}>
