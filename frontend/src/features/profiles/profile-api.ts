...
export async function saveDeveloperProfile(
  ...
  const experience = experienceYearsFrom(profile.experience);
  if (options.saveAccount !== false) {
  ...
  await upsert<DeveloperWire>("/developer-profile", { job_title: profile.jobTitle?.trim() || profile.role?.trim() || null, bio: profile.bio?.trim() || null, experience_years: experience, github: profile.github?.trim() || null, linkedin: profile.linkedin?.trim() || profile.linkedIn?.trim() || null, portfolio_link: profile.portfolioLink?.trim() || null, skills: profile.techStack ?? profile.skills ?? [], profile_complete: Boolean(profile.profileComplete), educations: educationPayload(profile.educations), certifications: (profile.certifications ?? []).map((item) => ({ name: typeof item === "string" ? item : item.name, issuer: "Not specified", issue_date: null, credential_id: null, credential_url: null })) }, { preferCreate: options.preferCreate });
